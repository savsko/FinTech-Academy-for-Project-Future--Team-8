...
public class WalletService {
    ...
    public void walletRequest(Map<String, Object> payload) { //ok
        rabbitTemplate.convertAndSend(AMQPConfiguration.exchangeName, AMQPConfiguration.walletRoutingKey, calculate(payload));
        log.info("A wallet payload has been sent to the queue.");
    }

    private Map<String, Object> calculate(Map<String, Object> payload) {
        ...
        if(Strings.isNullOrEmpty((String) payload.get("wallet_flag"))) {
            payload.put("wallet_flag", "");
        }
